...
export const getAuditLogs = async (req: Request, res: Response) => {
  try {
    ...
    if (action && typeof action === 'string') query = query.where('action', '==', action.toUpperCase());
    if (performedBy && typeof performedBy === 'string') query = query.where('performedBy', '==', performedBy);
    if (role && typeof role === 'string') query = query.where('role', '==', role.toUpperCase());
    if (entityType && typeof entityType === 'string') query = query.where('entityType', '==', entityType.toUpperCase());
    if (entityId && typeof entityId === 'string') query = query.where('entityId', '==', entityId);
...
export const getAuditLogsByEntity = async (req: Request, res: Response) => {
  try {
    const entityType = (req.params.entityType as string).toUpperCase();
