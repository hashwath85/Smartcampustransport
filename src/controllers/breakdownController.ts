...
export const reportBreakdown = async (req: Request, res: Response) => {
  try {
    ...
    await sendToTopic(
      `bus_${busId}`,
      '⚠️ Emergency Bus Breakdown',
      `Bus ${busId} reported: ${reason}. Support dispatched.`
    );
...
export const getAllBreakdowns = async (req: Request, res: Response) => {
  try {
    const { status, busId, severity } = req.query;
    let query: FirebaseFirestore.Query = db.collection('breakdowns');

    if (status && typeof status === 'string') query = query.where('status', '==', status);
    if (busId && typeof busId === 'string') query = query.where('busId', '==', busId);
    if (severity && typeof severity === 'string') query = query.where('severity', '==', severity);
...
export const getActiveBreakdowns = async (req: Request, res: Response) => {
  try {
...
export const getBreakdownById = async (req: Request, res: Response) => {
  try {
...
export const assignBackupBus = async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
...
export const resolveBreakdown = async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Breakdown ID required' });
